...
Function floattostr2(x:double):string;//use dot decimal seperator
function strtofloat_dot(s : string): double;//str to float for dot seperator
...
function strtofloat_local(s:string): double;{uses the current decimal seperator, either dot or komma, fault tolerant}
var
  error1:integer;
begin
  if local_DecimalSeparator<>'.' then s:=StringReplace(s,local_DecimalSeparator,'.',[]); {replaces komma by dot}
